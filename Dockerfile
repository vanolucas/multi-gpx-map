...
ENV VIRTUAL_ENV /opt/multi-gpx-map-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/multi-gpx-map/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/multi-gpx-map/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/multi_gpx_map/ && touch src/multi_gpx_map/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/multi-gpx-map/
...
# Expose the app.
ENTRYPOINT ["/opt/multi-gpx-map-env/bin/poe"]
CMD ["app"]
